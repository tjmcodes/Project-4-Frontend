...
function AllVenues() {
  ...
  return (
    <><NavBar /><div className=" bg-hero-pattern bg-cover bg-fixed min-h-screen bg-center">
      ...
      <div className=" desktop:p-5 desktop:m-2 mx-auto laptop:p-4 laptop:m-2 tablet:m-2 fold:p-1 fold:m-1">
        {data === false ? <div className=" mt-20 w-full flex items-center justify-around flex-col"><Loader /> <p className='text-white mt-5'>Venues loading please wait</p></div> :
          <div className="tablet:grid grid-cols-3">
            {venuesSearch().map((venue, index) => <div className="bg-cover laptop:bg-center rounded-xl desktop:m-5 tablet:m-1 fold:mb-7" key={index} style={{ backgroundImage: `url(${venue.backgroundCardImage})` }}>
